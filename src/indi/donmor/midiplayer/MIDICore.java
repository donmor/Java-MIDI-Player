...
public class MIDICore
{
			...
			case partial:
				...
				try
				{
					if (midiLoopStart < midiLoopEnd)
					{
						if (fps == Sequence.PPQ)
						{
							midip.setLoopStartPoint((long) (midiLoopStart * midip.getTempoInBPM()
									* sequence.getResolution() / 60000000));
							midip.setLoopEndPoint(
									(long) (midiLoopEnd * midip.getTempoInBPM() * sequence.getResolution() / 60000000));
						}
						else if (fps > Sequence.PPQ)
						{
							midip.setLoopStartPoint((long) (midiLoopStart * fps * sequence.getResolution() / 1000000));
							midip.setLoopEndPoint((long) (midiLoopEnd * fps * sequence.getResolution() / 1000000));
						}
						else
						{
							throw new Exception();
	...
	public void midiNavigate(long time)
	{
		...
		if (!((repeat == cycleType.partial && time >= midiLoopEnd) | time >= midip.getMicrosecondLength()))
		{
			if (time >= 0)
			{
